...
return ( <section id="work" className="py-24 px-[8vw] md:px-[7vw] lg:px-[16vw] font-sans relative z-10">
  ...
  >
    {projects.map((project) => (
      <motion.div variants={itemVariants} key={project.id}>
        <Tilt tiltMaxAngleX={5} tiltMaxAngleY={5} perspective={1000} transitionSpeed={1000} gyroscope={true} className="h-full">
            ...
            <div className="p-6 flex-grow flex flex-col relative z-20 -mt-6">
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                {project.tags.slice(0, 3).map((tag, index) => (
                  <span key={index} className="bg-primary/10 border border-primary/20 text-[10px] font-bold text-primary rounded-full px-3 py-1 uppercase tracking-wider">
                    {tag}
                  </span>
                ))}
  ...
  <AnimatePresence>
    {selectedProject && (
      <motion.div 
        ...
        className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8 backdrop-blur-md bg-black/30"
      >
        <motion.div 
          ...
          className="bg-background border border-black/10 rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col relative"
        >
            ...
            <div className="p-8 md:p-12 relative z-20 -mt-20">
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                {selectedProject.tags.map((tag, index) => (
                  <span key={index} className="bg-primary/10 border border-primary/20 text-xs font-bold text-primary rounded-full px-3 py-1 uppercase tracking-wider">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-2xl text-center text-lg font-bold transition-all hover:-translate-y-1"
                >
                  Live Demo
                </a>
              </div>
